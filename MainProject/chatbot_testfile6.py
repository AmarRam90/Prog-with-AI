...
def preprocess_query(query):
    ...
    # Extract lowercase tokens without punctuation
    tokens = [token.text.lower() for token in doc if token.is_alpha]
    
    return tokens

def get_device_details(query, mobile_data_df):
    # Tokenize the query
    query_tokens = preprocess_query(query.lower())

    # Find devices that match the query
    matching_devices = mobile_data_df[mobile_data_df['Name'].str.lower().str.contains('|'.join(query_tokens))]

    if not matching_devices.empty:
        # Display details of the first matching device
        device_details = matching_devices.iloc[0]
        response = f"Chatbot: Here are the details for {device_details['Name']}:\n\n"
        response += f"ID: {device_details['ID']}\n"
        response += f"Brand: {device_details['Brand']}\n"
        response += f"Price: {device_details['Price']}\n"
        response += f"Cleaned Price: {device_details['Cleaned_Price']}\n"
        response += f"Cleaned Rating: {device_details['Clean_Rating']}\n"
        response += f"Total Ratings: {device_details['Total Ratings']}\n"
        response += f"Rating: {device_details['Rating']}\n"
        response += f"Specification: {device_details['Specification']}\n"
        response += f"URL: {device_details['URL']}\n"

        return response
    else:
        return "Chatbot: No device details found for the query."

def get_two_best_devices(query, mobile_data_df):
    # Tokenize the query
    query_tokens = preprocess_query(query.lower())
    ...
    if not matching_devices.empty:
        ...
        table_html = '<table class="table table-bordered" style="max-width: 80%;"><thead><tr><th>ID</th><th>Name</th><th>Cleaned_Price</th><th>Clean_Rating</th><th>Total Ratings</th><th>Rating</th><th style="max-width: 150px;">URL</th></tr></thead><tbody>'

        for _, row in best_two_devices.iterrows():
            table_html += f'<tr><td>{row["ID"]}</td><td>{row["Name"]}</td><td>{row["Cleaned_Price"]}</td><td>{row["Clean_Rating"]}</td><td>{row["Total Ratings"]}</td><td>{row["Rating"]}</td><td style="max-width: 150px;"><a href="{row["URL"]}" target="_blank">Link</a></td></tr>'
...
def handle_intent(query, mobile_data_df):
    ...
    for token in doc:
        if any(greet in token.text.lower() for greet in ['hey', 'hello', 'hi']):
            ...
            break
        elif 'device details' in token.text.lower():
            intent = "device_details_query"
            break
        elif 'best devices' in token.text.lower():
            intent = "best_devices_query"
            break
    ...
    elif intent == "device_details_query":
        return get_device_details(query)
    elif intent == "best_devices_query":
        return get_two_best_devices(query)
    else:
